...
class ProbAttention(nn.Module):
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attn_mask):
        ...
        if self.output_attention:
            attns = (torch.ones([B, H, L_V, L_V], device=V.device) / L_V).type_as(attn).to(attn.device)
            attns[torch.arange(B, device=V.device)[:, None, None], torch.arange(H, device=V.device)[None, :, None], index, :] = attn
...
class ReformerLayer(nn.Module):
    def __init__(self, attention, d_model, n_heads, d_keys=None,
        ...
        try:
            ...
            raise ImportError("Please install reformer-pytorch: pip install reformer-pytorch")
